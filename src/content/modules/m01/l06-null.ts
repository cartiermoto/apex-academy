...
String missing = null;`,
    ...
    },
    {
      ...
      answer: 0,
      explain: {
        es: "isBlank() cubre null, cadena vacía y cadena de espacios de una vez. Y lead.Region__c.length() == 0, además, explotaría justo en el caso que intenta detectar: si Region__c es null, no hay a quién pedirle length().",
        en: "isBlank() covers null, empty string and whitespace-only in one go. And lead.Region__c.length() == 0 would also explode on the exact case it is trying to detect: if Region__c is null, there is nothing to ask for length().",
      },
      tags: ["interleaving"],
    },
    {
