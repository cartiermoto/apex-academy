...
export const l10Checkpoint: Lesson = {
  ...
  theory: [
    ...
    },
    {
      type: "p",
      text: {
        es: "3 · String y qué es un método. Un método es una acción con nombre que un dato sabe hacer consigo mismo: lo que en fórmulas era TRIM(Name), aquí es name.trim(). Devuelve siempre un valor nuevo y nunca modifica el original, así que String clean = raw.trim(); guarda el trabajo y raw.trim(); lo tira. Importa porque es el primer sitio donde se ve que en Apex los datos no son inertes: tienen comportamiento.",
        en: "3 · String and what a method is. A method is a named action a value knows how to perform on itself: what formulas wrote as TRIM(Name), here is name.trim(). It always returns a new value and never modifies the original, so String clean = raw.trim(); keeps the work and raw.trim(); throws it away. It matters because it is the first place you see that in Apex data is not inert: it has behaviour.",
      },
    },
    {
      type: "p",
